...
def event(organiser):
    ...
    with scopes_disabled():
        ...
        enable_plugin(event, "pretalx_vimeo")
        event.save()
...
@pytest.fixture
def schedule(event, submission, room):
    with scopes_disabled():
...
@pytest.fixture
def vimeo_link(submission):
    with scopes_disabled():
        return VimeoLink.objects.create(submission=submission, video_id="123456789")
